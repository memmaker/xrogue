...
{
    if (killer && !strncmp(killer, "a ", 2)) killer += 2;
    else if (killer && !strncmp(killer, "an ", 3)) killer += 3;
    else if (killer && !strncmp(killer, "the ", 4)) killer += 4;
    js_beacon("xrogue", ev, whoami, killer, level, (int) score, (int) turns, pstats.s_lvl);
}
